...
if (isset($request->post['submit'])) {
	try {
		...
		if (isset($_FILES["filename"]["type"])) {
			...
			if (in_array($file_extension, $validextensions)) {
				if ($_FILES["filename"]["error"] > 0) {
					throw new Exception("Código de Devolução: " . $_FILES['filename']['error']);
				} else {
		...
		foreach ($Sheets as $Index => $Name) {
			...
			switch ($Name) {
				case 'Product':
					foreach ($Reader as $Row) {
						...
						foreach ($store_code_names as $store_code) {
							if (!get_store_id_by_code($store_code)) {
								throw new Exception(trans('store_code ' . $store_code . ' não é válido!') . ' (' . $pro_data['product_name'] . '-' . $pro_data['code'] . ')');
							}
						}
					...
					break;
				default:
					throw new Exception('Página xls do (Produto) não é válida!');
					break;
			}
		}
		...
		$error = 0;
		$message = '';
		$message .= '<div><span class="fa fa-fw fa-info-circle"></span> Total de Itens: ' . $total_item_no . '</div>';
		if (count($insert_status) > 0) {
			...
			$message .= '<p><strong>Status de Inserção</strong></p>';
			$message .= '<ul>';
			$message .= '<li>Total Inserido: ' . $success . '</li>';
			$message .= '<li>Erro em: ' . $error . '</li>';
			$message .= '</ul>';
		}
		...
		if (count($update_status) > 0) {
			...
			$message .= '<p><strong>Status de Atualização</strong></p>';
			$message .= '<ul>';
			$message .= '<li>Total Atualizado: ' . $success . '</li>';
